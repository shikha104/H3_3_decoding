...
def get_k(alpha, beta):
    ...
    numerator = np.log(((1 - alpha) * (1 - beta)) / ((alpha * alpha) / 2))
    denominator = np.log((alpha / (2 * beta)))
    k = (numerator / denominator) + 1
    return k


def k_fill(decoded_sequence, source_sequence, alpha1, beta1):
    """
    Corrects a daughter sequence by filling stretches of zeros belonging to H3.1.
    Stretches must:
        - Be bordered by `1`s (from H3.1 or H3.3).
        - Contain ≤ k zeros (where k is dynamically computed using get_k).
    ...
    Parameters:
        ...
        alpha (float): Probability that a 1 will be followed by another 1.
        beta (float): Probability that a 0 will be followed by another 0.

    Returns:
        corrected_sequence (list): Corrected version of the daughter sequence.
    """
    k = get_k(alpha1, beta1)  # Compute dynamic k value
...
class Dataframe:
    ...
    def perform_kfill(self):
        ...
        for j in range(self.n_daughters):
            ...
            self.kfill_daughter_list[j] = k_fill(self.decoded_daughter_list[j], self.source_sequence, self.alpha1, self.beta1)
